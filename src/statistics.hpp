...
class StatisticsPool {
    std::map<std::string_view, Statistics> m_pool{};

public:
    void        add(Statistics&& s) noexcept { m_pool.emplace(s.name(), s); }
    Statistics& get(std::string_view name) noexcept { return m_pool[name]; }
};
